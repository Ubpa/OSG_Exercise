...
{
public:
    RotateCB(double oldAngle = 0.f) : _angle(oldAngle) {}
        ...
        if (picker->containsIntersections())
        {
            ...
            while (idx--)
            {
				...
				if (_selectedNode.valid()) {
					...
					RotateCB * oldRot = dynamic_cast<RotateCB*>(_selectedNode->getUpdateCallback());
					if (oldRot != NULL)
						oldAngle = oldRot->getAngle();
					_selectedNode->setUpdateCallback(NULL);
				}

                _selectedNode = mt;
                _selectedNode->setUpdateCallback( new RotateCB(oldAngle) );
                break;
            }
            if (!_selectedNode.valid())
